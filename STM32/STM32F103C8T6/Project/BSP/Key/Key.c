...
void Key_Init(void)
{
	...
	GPIO_InitStructure.GPIO_Pin = KEY_All_Pin;        // Configure PB0, PB1, PB10, PB11
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz; // Set speed to 50MHz
	GPIO_Init(KEY_PORT, &GPIO_InitStructure);         // Initialize GPIOB with the defined structure
}
...
void Key_Tick(void)
{
	...
	if (Key_Cnt >= 20)
	{
		...
		Keys[KEY_1].Key_State = GPIO_ReadInputDataBit(KEY_PORT, KEY_Pin_1); // 读取按键状态
		Keys[KEY_2].Key_State = GPIO_ReadInputDataBit(KEY_PORT, KEY_Pin_2); // 读取按键状态
		Keys[KEY_3].Key_State = GPIO_ReadInputDataBit(KEY_PORT, KEY_Pin_3); // 读取按键状态
		Keys[KEY_4].Key_State = GPIO_ReadInputDataBit(KEY_PORT, KEY_Pin_4); // 读取按键状态
