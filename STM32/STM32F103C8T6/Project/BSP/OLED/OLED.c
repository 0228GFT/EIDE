 ...
 * @brief OLED写SCL高低电平
 *
 * @param BitValue 要写入SCL的电平值，范围：0/1
 ...
 * @brief OLED写SDA高低电平
 *
 * @param BitValue 要写入SDA的电平值，范围：0/1
...
void OLED_GPIO_Init(void)
{
	...
	GPIO_InitStructure.GPIO_Pin = OLED_SCL_Pin; // SCL引脚
	GPIO_Init(OLED_SCL_Port, &GPIO_InitStructure);
	GPIO_InitStructure.GPIO_Pin = OLED_SDA_Pin; // SDA引脚
...
{
	// 一、开启时钟
	RCC_APB1PeriphClockCmd(OLED_I2C_RCC_Port, ENABLE);
	RCC_APB2PeriphClockCmd(OLED_I2C_RCC_GPIO_Port, ENABLE);
...
void OLED_WriteCommand(uint8_t Command)
{
	...
	OLED_WaitEvent(OLED_I2C_Port, I2C_EVENT_MASTER_MODE_SELECT);

	I2C_Send7bitAddress(OLED_I2C_Port, 0x78, I2C_Direction_Transmitter);
	OLED_WaitEvent(OLED_I2C_Port, I2C_EVENT_MASTER_TRANSMITTER_MODE_SELECTED);

	I2C_SendData(OLED_I2C_Port, 0x00);
	OLED_WaitEvent(OLED_I2C_Port, I2C_EVENT_MASTER_BYTE_TRANSMITTING);

	I2C_SendData(OLED_I2C_Port, Command);
	OLED_WaitEvent(OLED_I2C_Port, I2C_EVENT_MASTER_BYTE_TRANSMITTED);
...
void OLED_WriteData(uint8_t *Data, uint8_t Count)
{
	...
	for (i = 0; i < Count - 1; i++)
	{
		I2C_SendData(OLED_I2C_Port, Data[i]); // 依次发送Data的每一个数据
		OLED_WaitEvent(OLED_I2C_Port, I2C_EVENT_MASTER_BYTE_TRANSMITTING);
	}
 ...
 * @param X 指定点的x坐标
 * @param Y 指定点的y坐标
 * @param StartAngle 起始角度，范围：-180~180，水平向右为0度，水平向左为180度或-180度，下方为正数，上方为负数，顺时针旋转
 * @param EndAngle 终止角度，范围：-180~180，水平向右为0度，水平向左为180度或-180度，下方为正数，上方为负数，顺时针旋转
 ...
 * 	 如果未找到指定中文字符时，会显示默认图形（一个方框，内部一个问号）
...
void OLED_Printf(int16_t X, int16_t Y, uint8_t FontSize, char *format, ...)
{
	...
	OLED_ClearArea(X, Y, 128 - X, FontSize * 4 - 16); // 清除X坐标到屏幕最右侧，FontSize高度的区域
...
void OLED_Test(void)
{
	...
	OLED_Printf(0, 0, OLED_8X16, "OLED Test");
	OLED_Update();
}
